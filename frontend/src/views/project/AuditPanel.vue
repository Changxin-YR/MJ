...
import { api } from '../../api'
import type { Asset, AuditEvent, Project } from '../../types'
...
async function load() { try { if (props.canAudit) logs.value = await api.get(`${root.value}/audit`); assets.value = await api.get(`${root.value}/assets`) } catch (cause) { error.value = (cause as Error).message } }
...
<template>
  ...
  <section class="panel panel-pad" style="margin-bottom:18px"><h2>成片溯源</h2><div class="row"><select v-model="selectedAssetId" aria-label="选择成片素材"><option value="" disabled>选择最终 MP4 素材</option><option v-for="asset in assets.filter(a => a.mime === 'video/mp4')" :key="asset.id" :value="asset.id">{{ asset.id.slice(0,12) }} · {{ asset.duration?.toFixed(1) }} 秒</option></select><button class="btn" @click="loadLineage"><Search :size="15" />查看链路</button></div><pre v-if="lineage" class="subtle-panel mono" style="padding:15px;white-space:pre-wrap;overflow:auto;max-height:350px;margin-top:14px">{{ JSON.stringify(lineage,null,2) }}</pre></section>
