...
真实 Provider 运行脚本：`backend/scripts/provider_smoke.py` 验证模型契约和文件；`integrated_real_smoke.py` 验证 API→Job→Worker→Inspector→MinIO；`final_demo.py` 生成长片；`embedding_smoke.py` 除验证真实向量索引与查询外，还会执行一组中文连续对白 probe，要求“钥匙对白”与查询的余弦相似度高于无关“药田对白”，并验证省略主语对白块能够保留角色上下文；`director_llm_smoke.py` 验证真实 LLM 与 RAG；`inspect_existing_videos.py` 保存逐镜头视觉检查；`rerender_demo.py` 使用已有素材重新渲染。真实模型调用需要 `DASHSCOPE_API_KEY`，不会把密钥写入证据，并会产生费用，不纳入每次 CI。
