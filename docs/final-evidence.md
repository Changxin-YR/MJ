...
更新于 2026-09-25。状态只反映已执行的测试与运行证据；`PASS` 表示该行列出的范围已验证，不表示整个冻结设计已完成。
...
| 中文 RAG、对白上下文与项目隔离 | `app/rag/service.py`、`app/providers/embeddings.py` | `test_rag_scope.py`、`test_provider_contract.py` | 中文句/段边界切块、连续对白窗口、邻接上下文、向量+词面轻量重排、重复索引唯一性、失败重建保留旧索引、Project Scope 隔离及 text-embedding-v4 批处理契约均有自动化覆盖 | PASS（自动化） |
| 真实向量与语义检索 | `app/providers/embeddings.py`、`app/rag/service.py` | `scripts/embedding_smoke.py` | [embedding.json](../evidence/final-demo/embedding.json)：`text-embedding-v4`，1024 维，真实查询返回本项目来源；本轮新增 10 条/请求的批量索引路径已由模拟 HTTP 契约测试覆盖 | PASS（真实单项目旧样本 + 新批处理自动化） |
...
| 前端构建与核心 E2E | `frontend/src/`、`frontend/e2e/` | `npm run build`、`npm run e2e` | GitHub Actions run #127：Vue production build 通过；注册→角色激活→角色绑定分镜→生成→审核→时间线→播放成片，以及 Director→PendingAction→Approve→Resume 两条浏览器流程通过 | PASS（2 条 E2E） |
...
| 安全、并发与故障矩阵 | `backend/tests/` | 权限/Scope、中文 RAG、并发预算与编号、唯一 Timeline/KnowledgeDocument、工具权限、上传、Prompt Injection、Callback Replay、FFmpeg 注入、幂等、Provider 熔断与临时/永久失败、语言硬门禁、派生素材失效 | GitHub Actions run #127：53 个后端测试通过；Ruff 与 `alembic check` 通过 | PASS |
...
当前自动化门禁以 GitHub Actions run #127 为最新已完成基线：53 个后端测试、Ruff/Alembic、Vue production build 和 2 条 Playwright E2E 全部通过。历史真实 Provider、ComfyUI 与 60 秒成片证据仍有效地证明旧真实链路，但本轮新增的四帧 Inspector、中文对白 RAG、Embedding 批处理和 MUSIC/SFX 混音没有重新产生付费 Provider 全链路证据。因此当前结论为“本轮代码与自动化验收通过；新增真实模型策略与完整用户手动试用仍待最终复验”。Director 的长期自动记忆回写仍明确处于 `DEFERRED_UNTIL_CANONICAL_APPROVAL`，不计入已完成功能。
